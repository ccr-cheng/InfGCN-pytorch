...
class GCNLayer(nn.Module):
    def __init__(self, irreps_in, irreps_out, irreps_edge, radial_embed_size, num_radial_layer, radial_hidden_size,
                 is_fc=True, use_sc=True, irrep_normalization='component', path_normalization='element'):
        r"""
...
class InfGCN(nn.Module):
    def __init__(self, n_atom_type, num_radial, num_spherical, radial_embed_size, radial_hidden_size,
                 num_radial_layer=2, num_gcn_layer=3, cutoff=3.0, grid_cutoff=3.0, is_fc=True,
                 gauss_start=0.5, gauss_end=5.0, activation='norm', residual=True, pbc=False, **kwargs):
        """
    ...
    def forward(self, atom_types, atom_coord, grid, batch, infos):
        ...
        for i, gcn in enumerate(self.gcns):
            feat = gcn(edge_index, feat, edge_feat, edge_embed, dim_size=atom_types.size(0))
        ...
        if self.residual:
            ...
            grid_batch = torch.arange(n_graph, device=grid.device).repeat_interleave(n_sample)
            grid_dst, node_src = radius(atom_coord, grid_flat, self.grid_cutoff, batch, grid_batch)
